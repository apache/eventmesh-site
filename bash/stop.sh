...
# Detect operating systema
OS=$(uname)

EVENTMESH_HOME="/root/apache-eventmesh-1.9.0-src"

export EVENTMESH_HOME

function get_pid {
	local ppid=""
	if [ -f ${EVENTMESH_HOME}/pid.file ]; then
		ppid=$(cat ${EVENTMESH_HOME}/pid.file)
