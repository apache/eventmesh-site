...
- 文件加载：通过将插件安装到插件目录，EventMesh 在运行时会根据条件自动加载插件目录下的插件。请参考 [2.3 构建](#2.3-构建) 和 [2.4 打包插件](#2.4-打包插件)。

>当您对源代码作出更改后，建议在 [2.3 构建](#2.3-构建) 给出的命令中添加`build`任务，以重新编译和运行单元测试。如：
>
>```shell
